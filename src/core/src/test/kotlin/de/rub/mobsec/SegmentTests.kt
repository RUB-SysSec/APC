...
internal class SegmentTests {
    @TestFactory
    fun getMove() = testRangeNoZero.flatMap { x1 ->
    ...
    @TestFactory
    fun getMaxHorizontalOrVerticalDistance() = testRangeNoZero.flatMap { x1 ->
        testRangeNoZero.flatMap { y1 ->
            testRangeNoZero.flatMap { x2 ->
                testRangeNoZero.map { y2 ->
                    dynamicTest("$x1,$y1 $x2,$y2") {
                        assertEquals(
                            max(abs(x2 - x1), abs(y2 - y1)), Segment[Node[x1, y1], Node[x2, y2]].maximumNorm
                        )
                    }
                }
            }
    ...
    @TestFactory
    fun getPointsBetweenAB() = testRange.flatMap { x ->
        testRange.flatMap { y ->
            allMoves.flatMap {
                val point = Node[x, y]
                sequenceOf(dynamicTest("1") {
                    ...
                    assertIterableEquals(
                        setOf(point - it, point), Segment[point - it - it, point + it].nodesBetweenAB
                    )
                })
            }
        }
    ...
    @TestFactory
    fun isSinglePoint() = testRangeNoZero.flatMap { x1 ->
        testRangeNoZero.flatMap { y1 ->
            testRangeNoZero.flatMap { x2 ->
                testRangeNoZero.map { y2 ->
                    dynamicTest("$x1,$y1 $x2,$y2") {
    ...
    @TestFactory
    fun isKnightMove() = testRange.flatMap { x ->
        testRange.flatMap { y ->
            knightMoves.map {
                dynamicTest("#1 $x,$y") {
    ...
    @TestFactory
    fun getEuclideanDistance() = testRange.flatMap { x ->
        testRange.flatMap { y ->
    ...
    @TestFactory
    fun isConsecutive() = testRangeNoZero.flatMap { x1 ->
        testRangeNoZero.flatMap { y1 ->
            testRangeNoZero.flatMap { x2 ->
                testRangeNoZero.flatMap { y2 ->
                    sequenceOf(dynamicTest("1 true") {
    ...
    @TestFactory
    fun isValidWith() = testRangeNoZero.flatMap { x ->
    ...
    @TestFactory
    fun isValidWithLast() = testRangeNoZero.flatMap { x ->
        testRangeNoZero.flatMap { y ->
            allMoves.flatMap {
                val segment = Segment[Node[x, y], Node[x, y] + it * 2]
    ...
    @TestFactory
    fun isTurn() = testRangeNoZero.flatMap { x ->
        testRangeNoZero.flatMap { y ->
            normalMoves.flatMap {
    ...
    @TestFactory
    fun isSunIntersecting() = smallTestRange.flatMap { x ->
        smallTestRange.flatMap { y ->
            sequenceOf(dynamicTest("1 - small x") {
                assertAll(
    ...
    @TestFactory
    fun isSongIntersecting() = smallTestRange.flatMap { x ->
        smallTestRange.flatMap { y ->
            sequenceOf(dynamicTest("1 - small x") {
                assertAll(
    ...
    @TestFactory
    fun isOverlapping() = testRangeNoZero.flatMap { x ->
        testRangeNoZero.flatMap { y ->
            allMoves.flatMap {
                sequenceOf(dynamicTest("1") {
    ...
    @TestFactory
    fun isOverlapping1() = (testRange.flatMap { x ->
        testRange.flatMap { y ->
            sequenceOf(dynamicTest("1") {
